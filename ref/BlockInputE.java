  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
        try (BufferedReader br = new BufferedReader(new FileReader(FILENAME))) {
            ...
            // JAX and Marshaller will do the dirty work for us, serializing XML data and reconstructing on the other end
            ...
            // For each line in the file on disk, create a new block record
            ...
            for(int i=0; i < n; i++){
                System.out.println("  " + blockArray[i].getFFname() + " " + blockArray[i].getFLname());
            }
            System.out.println("\n");

            stringXML = sw.toString();

            // For each entry in the BlockRecord array, marshal it into a StringWriter
            // I think basically we're just concatenating them into one single collection
            for(int i=0; i < n; i++){
                jaxbMarshaller.marshal(blockArray[i], sw);
            }

            // Get the raw string of all of the BlockRecord entries, concatenated into the StringWriter
            // Then, since they were all given an individual header, remove them
            // @TODO I think the final thing to do is add a single XML header at the top of the string
            ...
            // @TODO we created and concatenated all the individual XML blocks, but we still need to now multicast them
